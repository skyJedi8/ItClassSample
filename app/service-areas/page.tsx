    ...
    <main>
      <Section>
        ...
        <div className="mt-9 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {areas.map((area) => (
            <Link key={area} className="glass-card group p-6 transition hover:-translate-y-1 hover:border-brand-200/50" href={`/service-areas/${area.toLowerCase().replace(' ', '-')}`}>
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-brand-200">Service area</p>
              <h2 className="mt-2 text-2xl font-semibold text-white">{area}, TX</h2>
