    ...
    <div className="grid gap-5 md:grid-cols-2">
      {pairs.map((pair) => (
        <article key={pair.label} className="premium-panel p-4">
          <p className="text-xs font-semibold uppercase tracking-[0.15em] text-sky-200">Real Results</p>
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <Image src={`/placeholders/${pair.after}`} alt="After cleaning" width={800} height={600} className="h-36 w-full rounded-lg border border-sky-500/50 object-cover" />
            </div>
          </div>
        </article>
      ))}
